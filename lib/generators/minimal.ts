...
export const minimalPreview = ({
  ...
  ctx.fillStyle = colors.subtext1
  ctx.fillText("Made by Jason Cameron", canvas.width - 250, canvas.height - 20)
  ...
  const githubIconX = canvas.width - 40
  const githubIconY = canvas.height - 32

  ctx.save()
  ctx.translate(githubIconX, githubIconY)
  ctx.scale(githubIconSize / 24, githubIconSize / 24)
  const path = new Path2D(githubIcon)
  ctx.fill(path)
  ctx.restore()
